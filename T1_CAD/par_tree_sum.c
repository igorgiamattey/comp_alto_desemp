...
long sum_tree_par(Node* node, int depth) {
    ...
    long *sum_tree_arr = (long *)calloc(node->num_children, sizeof(long));

    for (int i = 0; i < node->num_children; i++) {
        #pragma omp task shared(sum_tree_arr)
        sum_tree_arr[i] = sum_tree_par(node->children[i], depth+1);
    }

    #pragma omp taskwait

    for (int i = 0; i < node->num_children; i++) {
        total_sum += sum_tree_arr[i];
    }
